                        ...
                        pushd ${path_to_build}/binary
                            ...
                            for dist in `ls -1 debian`; do
                                for deb in `find debian/\${dist} -name '*.deb'`; do
                                    repopush --remove-package --gpg-pass ${SIGN_PASSWORD} --package \${deb} --verbose --component ${DESTINATION} --codename \${dist} --repo-path /srv/repo-copy/tools/apt
                                done

                                # source deb
                                #for dsc in `find ../source -name '*.dsc'`; do
                                #    repopush --remove-package --gpg-pass ${SIGN_PASSWORD} --package \${dsc} --verbose --component ${DESTINATION} --codename \${dist} --repo-path /srv/repo-copy/tools/apt
                                #done
                            done
                        popd
